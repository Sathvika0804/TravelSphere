  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/places/save?destination=${destination}`,
        {
          method: "POST",
      ...
      if (res.ok) {
        ...
        // Refresh places
        fetch(`http://localhost:8080/places/${destination}`)
          .then((res) => res.json())
          .then((data) => setPlaces(data));
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-100 to-purple-200 p-6 relative overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Famous Places in {destination}</h1>
        <div className="space-x-3">
          ...
          <button
            onClick={() => {
              ...
              if (!uid) {
                alert("⚠️ You must log in to add a place!");
                navigate("/signin"); // redirect to login
                return;
              }
              setShowAddModal(true); // only open modal if logged in
            }}
